...
const statusConfig: Record<string, { label: string; className: string }> = {
  ...
  approved: { label: "Active", className: "bg-green-100 text-green-800 border-green-200" },
...
export const PrescriptionCard: React.FC = () => {
  ...
  if (prescriptions.length === 0) {
    return (
      <Card className="bg-white/95 backdrop-blur-md shadow-lg">
        <CardHeader>
          <CardTitle className="text-lg font-manrope">My Prescription</CardTitle>
        </CardHeader>
        <CardContent className="flex flex-col items-center justify-center py-8">
          <Pill className="size-8 text-gray-300 mb-3" />
          <p className="text-gray-500 mb-4 text-center text-sm">No prescriptions yet</p>
      ...
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-gray-800">
            {current.medicationName || "Pending medication"}
          </span>
          <Badge className={status.className}>{status.label}</Badge>
        </div>

        {current.dosage && (
          <p className="text-sm text-gray-600">Dosage: {current.dosage}</p>
        )}

        <div className="grid grid-cols-2 gap-2 text-xs text-gray-500">
          <div>
            <span className="block text-gray-400">Date Issued</span>
            {formatDate(current.dateIssued)}
          </div>
          <div>
            <span className="block text-gray-400">Valid Till</span>
